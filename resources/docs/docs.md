...
The `home-routes` handler in the `nextdoor-store-api.routes.home` namespace
...
The `home-page` function will in turn call the `nextdoor-store-api.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `nextdoor-store-api.handler` namespace:
...
Request middleware functions are located under the `nextdoor-store-api.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `nextdoor-store-api.dev-middleware` namespace found in
